...
export default function AnalyticsDashboard({ onBack }) {
    ...
    return <main className="min-h-screen bg-[#f8fafc] px-4 py-8 text-slate-900 sm:px-8"><div className="mx-auto max-w-6xl">
        <header className="mb-8 flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><p className="text-sm font-semibold text-indigo-600">ArthMitra AI · Financial Services</p><h1 className="mt-1 text-4xl font-bold text-slate-900">Call Analytics</h1><p className="mt-3 max-w-3xl text-slate-600">Every browser call is recorded when it ends. A success means the caller received an eligibility answer or a scheme document list. This view contains aggregate, privacy-safe call data only.</p></div><button onClick={onBack} className="rounded-xl border border-indigo-200 px-4 py-2 text-sm font-semibold text-indigo-600 hover:bg-indigo-50">Back to calls</button></header>
        <div className="mb-8 flex items-center gap-3"><button onClick={load} disabled={loading} className="rounded-xl bg-[#4f46e5] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#4338ca] disabled:opacity-60">{loading ? "Refreshing…" : "Refresh now"}</button>{updatedAt && <span className="text-sm text-slate-500">Updated {updatedAt.toLocaleTimeString()}</span>}</div>
        {error ? <div className="rounded-2xl border border-rose-300 bg-rose-50 p-5 text-rose-700">{error}</div> : <>
            ...
            <section className="mt-5 grid gap-5 lg:grid-cols-2"><article className="rounded-3xl border border-slate-200 bg-white p-6"><h2 className="text-lg font-bold text-slate-900">By channel · चैनल</h2><div className="mt-5 flex items-center gap-4"><span className="w-20 text-slate-600">Browser</span><div className="h-5 flex-1 overflow-hidden rounded-full bg-rose-200"><div className="h-full bg-indigo-600" style={{ width: `${browser?.total_calls ? (browser.successful_calls / browser.total_calls) * 100 : 0}%` }} /></div><span className="font-semibold text-slate-800">{browser?.successful_calls || 0} / {browser?.total_calls || 0}</span></div></article><article className="rounded-3xl border border-slate-200 bg-white p-6"><h2 className="text-lg font-bold text-slate-900">Why calls failed · असफल क्यों</h2><div className="mt-4 space-y-2 text-slate-600"><p className="flex justify-between"><span>No response</span><b>{data?.failure_reasons?.no_response || 0}</b></p><p className="flex justify-between"><span>Incomplete</span><b>{data?.failure_reasons?.incomplete || 0}</b></p></div></article></section>
            <section className="mt-5 overflow-hidden rounded-3xl border border-slate-200 bg-white"><div className="flex items-center justify-between p-6"><h2 className="text-lg font-bold text-slate-900">Recent calls · हाल की कॉलें</h2><span className="text-sm text-slate-500">Newest first</span></div><div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead className="border-y border-slate-200 bg-slate-50 text-xs uppercase tracking-wider text-slate-500"><tr><th className="px-6 py-4">When</th><th>Channel</th><th>Duration</th><th>Outcome</th><th>Why</th></tr></thead><tbody>{data?.recent_calls?.length ? data.recent_calls.map((call, index) => <tr key={index} className="border-b border-slate-100"><td className="px-6 py-4">{when(call.started_at)}</td><td className="capitalize">{call.channel}</td><td>{duration(call.duration_seconds)}</td><td><span className={`rounded-full px-3 py-1 text-xs font-bold ${call.outcome === "successful" ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}>{call.outcome === "successful" ? "Successful" : "Failed"}</span></td><td>{call.completion_kind === "document_list" ? "Document list provided" : call.completion_kind === "eligibility_check" ? "Eligibility answer provided" : call.outcome === "successful" ? "ArthMitra response provided" : call.failure_reason === "no_response" ? "No response" : "Incomplete"}</td></tr>) : <tr><td className="px-6 py-8 text-slate-500" colSpan="5">No completed calls yet. Make and end a browser call to see it here.</td></tr>}</tbody></table></div></section>
        </>}</div></main>;
}
